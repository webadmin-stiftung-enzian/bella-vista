...
// WP-Optimize advanced-cache.php (written by version: 4.5.0) (homeurl: http://bella-vista.local/) (abspath: /Users/adomaitis/repos/bella_vista/app/public/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/Users/adomaitis/repos/bella_vista/app/public/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://bella-vista.local/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/Users/adomaitis/repos/bella_vista/app/public/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-bella-vista.local.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-bella-vista.local.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-bella-vista.local.php');
}
